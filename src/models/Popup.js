...
export default class Popup extends Base {
  ...
  */
  ...
  addCustomStyle(hashStr, palette, prefix) {
      ...
      if (button) {
        ...
        if (button.background != 'transparent') {
          colorStyles[
            ...
            'background-color: ' +
              (button.hover || getHoverColor(button.background))
          ]
        }
  ...
  removeCustomStyle(palette) {
    const hashStr = hash(JSON.stringify(palette))
    const customStyle = this.customStyles[hashStr]
